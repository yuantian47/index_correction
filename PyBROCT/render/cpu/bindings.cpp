...
void _set_volume_helper(CpuRenderer& r, py::array_t<int8_t, py::array::c_style | py::array::forcecast>& data, bool filter) {
    if(data.ndim() != 3) {
...
void _set_transfer_function_helper(CpuRenderer& r, py::array_t<float, py::array::c_style | py::array::forcecast>& data, float offset, float scale) {
...
auto _render_helper(CpuRenderer& r, const Eigen::Matrix4f& modelview, const Eigen::Matrix4f& projection, const pair_t<size_t>& resolution, const pair_t<pair_t<float>>& region) {
...
PYBIND11_MODULE(cpu, m) {
    ...
    py::class_<CpuRenderer>(m, "CpuRenderer")
        .def(py::init<>())

        .def("set_volume", &_set_volume_helper)
        .def("set_volume", [](CpuRenderer& r, py::array_t<int8_t, py::array::c_style | py::array::forcecast>& data) { _set_volume_helper(r, data, false); })

        .def("set_threshold", &CpuRenderer::set_threshold)
        .def("set_brightness", &CpuRenderer::set_brightness)
        .def("set_range", &CpuRenderer::set_range)
        .def("set_enhancement", &CpuRenderer::set_enhancement)

        .def("set_transfer_function", &_set_transfer_function_helper)
        .def("set_transfer_function", [](CpuRenderer& r, py::array_t<float, py::array::c_style | py::array::forcecast>& data) { _set_transfer_function_helper(r, data, 0, 1); })

        .def("set_cut_planes", [](CpuRenderer& r, const std::vector<pair_t<triple_t<float>>> planes) {
        ...
        .def("render", [](CpuRenderer& r, const Eigen::Matrix4f& modelview, const Eigen::Matrix4f& projection, const pair_t<size_t>& resolution) { _render_helper(r, modelview, projection, resolution, { {-1.0f, 1.0f}, {-1.0f, 1.0f} }); })

        .def_property_readonly_static("type", [](py::object&) { return "CPU"; })
    ;
